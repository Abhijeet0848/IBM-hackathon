...
def render_pomodoro_timer():
    ...
    """
    return (
        '<div id="pomodoro-root-card" style="background: linear-gradient(135deg, #ffffff 0%, #f8faff 100%); border: 1.5px solid #e2e8f0; border-radius: 18px; padding: 1.25rem 1.5rem; margin-bottom: 1.5rem; box-shadow: 0 10px 25px -5px rgba(15, 23, 42, 0.04), 0 4px 6px -2px rgba(15, 23, 42, 0.02);">'
        '<div style="display: flex; align-items: center; justify-content: space-between; border-bottom: 1px solid #f1f5f9; padding-bottom: 0.75rem; margin-bottom: 1rem; flex-wrap: wrap; gap: 10px;">'
        '<div style="display: flex; align-items: center; gap: 10px;">'
        '<span style="font-size: 1.35rem;">⏱️</span>'
        '<div>'
        '<div style="font-family: \'Outfit\', sans-serif; font-size: 1.12rem; font-weight: 800; color: #0f172a;">Pomodoro Focus Arena</div>'
        '<div style="font-size: 0.78rem; color: #64748b;">Execute targeted study blocks with ambient focus audio</div>'
        '</div>'
        '</div>'
        '<div id="pomo-mode-badge" style="background: #eef2ff; color: #4f46e5; border: 1px solid #c7d2fe; border-radius: 20px; padding: 0.25rem 0.75rem; font-size: 0.76rem; font-weight: 700; text-transform: uppercase; letter-spacing: 0.04em;">'
        '🍅 25m Focus Block'
        '</div>'
        '</div>'
        ...
        '<circle id="pomo-progress-circle" cx="85" cy="85" r="72" stroke="url(#pomoGradient)" stroke-width="10" stroke-linecap="round" fill="transparent" stroke-dasharray="452.39" stroke-dashoffset="0" style="transition: stroke-dashoffset 0.85s linear;" />'
        '<defs>'
        ...
        '<div id="pomo-timer-display" style="font-family: \'Outfit\', monospace; font-size: 2.2rem; font-weight: 800; color: #0f172a; line-height: 1;">25:00</div>'
        '<div id="pomo-status-label" style="font-size: 0.72rem; font-weight: 700; color: #64748b; text-transform: uppercase; letter-spacing: 0.05em; margin-top: 4px;">Ready to Focus</div>'
        '</div>'
        '</div>'
        '<div style="display: flex; flex-direction: column; gap: 0.75rem;">'
        '<div style="display: flex; gap: 6px; flex-wrap: wrap;">'
        '<button type="button" onclick="setPomodoroMode(25, \'🍅 25m Focus Block\', \'#4f46e5\', \'#eef2ff\')" style="flex: 1; min-width: 90px; background: #ffffff; border: 1.5px solid #c7d2fe; border-radius: 9px; padding: 0.4rem 0.5rem; font-size: 0.78rem; font-weight: 700; color: #4338ca; cursor: pointer;">🍅 25m Focus</button>'
        '<button type="button" onclick="setPomodoroMode(50, \'⚡ 50m Deep Work\', \'#7e22ce\', \'#faf5ff\')" style="flex: 1; min-width: 90px; background: #ffffff; border: 1.5px solid #e9d5ff; border-radius: 9px; padding: 0.4rem 0.5rem; font-size: 0.78rem; font-weight: 700; color: #6b21a8; cursor: pointer;">⚡ 50m Deep</button>'
        '<button type="button" onclick="setPomodoroMode(5, \'☕ 5m Short Break\', \'#15803d\', \'#f0fdf4\')" style="flex: 1; min-width: 90px; background: #ffffff; border: 1.5px solid #bbf7d0; border-radius: 9px; padding: 0.4rem 0.5rem; font-size: 0.78rem; font-weight: 700; color: #166534; cursor: pointer;">☕ 5m Break</button>'
        '</div>'
        '<div style="display: flex; gap: 8px;">'
        '<button id="pomo-toggle-btn" type="button" onclick="togglePomodoroTimer()" style="flex: 2; background: linear-gradient(135deg, #4f46e5 0%, #3b82f6 100%); color: #ffffff; border: none; border-radius: 10px; padding: 0.6rem 1rem; font-size: 0.92rem; font-weight: 700; cursor: pointer; box-shadow: 0 4px 12px rgba(79, 70, 229, 0.28);">▶️ Start Focus Block</button>'
        '<button type="button" onclick="resetPomodoroTimer()" style="flex: 1; background: #ffffff; color: #64748b; border: 1.5px solid #cbd5e1; border-radius: 10px; padding: 0.6rem 0.75rem; font-size: 0.88rem; font-weight: 600; cursor: pointer;">🔄 Reset</button>'
        '</div>'
        '<div style="display: flex; align-items: center; justify-content: space-between; background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 10px; padding: 0.45rem 0.85rem;">'
        '<span style="font-size: 0.78rem; font-weight: 700; color: #475569;">🎧 Ambient Audio:</span>'
        '<select id="pomo-audio-select" onchange="switchAmbientSound(this.value)" style="font-size: 0.76rem; font-weight: 600; border: 1px solid #cbd5e1; border-radius: 6px; padding: 2px 6px; background: #ffffff; color: #1e293b; outline: none;">'
        '<option value="none">🔇 Off (Silent)</option>'
        '<option value="rain">🌧️ Gentle Rain</option>'
        '<option value="cafe">☕ Coffee Shop Hum</option>'
        '<option value="ocean">🌊 Ocean Calm</option>'
        '</select>'
        '</div>'
        '</div>'
        '</div>'
        '<div id="pomo-celebrate-box" style="display: none; margin-top: 1rem; background: #f0fdf4; border: 1.5px solid #86efac; border-radius: 10px; padding: 0.65rem 1rem; text-align: center; color: #15803d; font-size: 0.88rem; font-weight: 700; animation: fadeIn 0.4s ease;">'
        '🎉 Fantastic focus session! Take a well-deserved breather.'
        '</div>'
        '</div>'
        '<script>'
        '(function() {'
        ...
        '  var gainNode = null;'
        '  '
        '  window.setPomodoroMode = function(mins, label, color, bg) {'
        ...
        '      if (status) status.textContent = "Deep Focus Active";'
        ...
        '  }'
        '  '
        '  window.switchAmbientSound = function(type) {'
        '    stopAmbientSound();'
