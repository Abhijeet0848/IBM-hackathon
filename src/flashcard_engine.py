...
class FlashcardEngine:
    ...
    def render_interactive_flashcard_deck(flashcards: List[Dict[str, Any]]) -> str:
        ...
        return (
            ...
            'body { font-family: "Plus Jakarta Sans", sans-serif; background: transparent; padding: 4px; }'
            '.deck-container { background: #ffffff; border: 1.5px solid #e2e8f0; border-radius: 20px; padding: 1.35rem 1.6rem; box-shadow: 0 10px 25px -5px rgba(15, 23, 42, 0.04); }'
            '.deck-header { display: flex; align-items: center; justify-content: space-between; border-bottom: 1px solid #f1f5f9; padding-bottom: 0.85rem; margin-bottom: 1.15rem; flex-wrap: wrap; gap: 10px; }'
            '.deck-counter { font-size: 0.82rem; font-weight: 700; color: #64748b; }'
            '.flip-card-stage { perspective: 1200px; width: 100%; min-height: 240px; margin-bottom: 1.25rem; cursor: pointer; }'
            '.flip-card-inner { position: relative; width: 100%; min-height: 240px; text-align: center; transition: transform 0.6s cubic-bezier(0.4, 0, 0.2, 1); transform-style: preserve-3d; }'
            '.flip-card-inner.flipped { transform: rotateY(180deg); }'
            '.flip-card-front, .flip-card-back { position: absolute; width: 100%; height: 100%; min-height: 240px; -webkit-backface-visibility: hidden; backface-visibility: hidden; border-radius: 16px; padding: 1.75rem 1.5rem; display: flex; flex-direction: column; justify-content: space-between; }'
            '.flip-card-front { background: linear-gradient(135deg, #f8faff 0%, #eef2ff 100%); border: 2px solid #c7d2fe; box-shadow: 0 8px 20px -4px rgba(99, 102, 241, 0.12); }'
            '.flip-card-back { background: linear-gradient(135deg, #f0fdf4 0%, #dcfce7 100%); border: 2px solid #86efac; transform: rotateY(180deg); box-shadow: 0 8px 20px -4px rgba(34, 197, 94, 0.12); text-align: left; }'
            '.tag-badge { align-self: flex-start; background: #ffffff; border: 1px solid rgba(0,0,0,0.08); padding: 0.2rem 0.6rem; border-radius: 20px; font-size: 0.72rem; font-weight: 700; text-transform: uppercase; letter-spacing: 0.04em; }'
            '.card-content-front { font-family: "Outfit", sans-serif; font-size: 1.3rem; font-weight: 700; color: #1e1b4b; line-height: 1.4; margin: auto 0; }'
            '.card-content-back { font-size: 1.05rem; font-weight: 600; color: #14532d; line-height: 1.55; margin: auto 0; }'
            '.deck-actions { display: flex; gap: 10px; align-items: center; justify-content: space-between; flex-wrap: wrap; }'
            '.btn-card { flex: 1; min-width: 130px; border-radius: 10px; padding: 0.65rem 1rem; font-size: 0.88rem; font-weight: 700; cursor: pointer; border: none; transition: all 0.15s ease; display: flex; align-items: center; justify-content: center; gap: 6px; }'
            ...
            '.progress-track { width: 100%; height: 6px; background: #f1f5f9; border-radius: 10px; overflow: hidden; margin-bottom: 1.15rem; }'
